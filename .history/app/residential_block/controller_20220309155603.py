	...
	def get_map_view(self, params: Munch, **kwargs) -> MapResult:
		...
		if params.tab1.section2.point:
			...
			for house in range(0,n_houses,1):
				...
				#Top left corner:
				if house ==0: #Only computed for the case of 1 house
					Top_Left_x = center_x - ((width) * cos(angle)) - ((height) * sin(angle))
					Top_Left_y = center_y - ((width) * sin(angle)) + ((height) * cos(angle))
					coords.append((Top_Left_x,Top_Left_y))
				
				#Top right corner:
				Top_Right_x = center_x + ((width*(2*house+1)) * cos(angle)) - ((height) * sin(angle))
				Top_Right_y = center_y + ((width*(2*house+1)) * sin(angle)) + ((height) * cos(angle))
				coords.append((Top_Right_x,Top_Right_y))

				#Bottom right corner:
				Bot_Right_x = center_x + ((width*(2*house+1)) * cos(angle)) + ((height) * sin(angle))
				Bot_Right_y = center_y + ((width*(2*house+1)) * sin(angle)) - ((height) * cos(angle))
				coords.append((Bot_Right_x,Bot_Right_y))
