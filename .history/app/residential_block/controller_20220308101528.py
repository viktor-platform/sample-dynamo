...
from ..lib.dynamo import stl_to_mesh, update_dynamo_input_values, get_output_data
...
class ResidentialBlockController(ViktorController):
	...
	@DataView("Building", duration_guess=5)
	def geometry_and_data_view(self, params, **kwargs):
		...
		dynamo_output_for_dataitem = {"(OUTPUT) Floor area per house": "Floor area per house"}
		data_items = get_output_data(dynamo_output_for_dataitem, dynamo_script, data_output.encode())

		return DataResult(DataGroup(*data_items))
	...
	def _map_params_to_dynamo_inputs(params):
		...
		return {
			...
			"Depth": params.tab1.section1.section1.depth,
