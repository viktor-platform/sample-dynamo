...
#
# dynamo_script_alt (vervallen, heb ik dit nog nodig?)
#
#
#
#
#
# @memoize
# def run_dynamo_script_alt(dynamo_script: dict) -> Tuple[str, str]:
#     try:
#         generic_analysis = GenericAnalysis(files=[("script.dyn", BytesIO(json.dumps(dynamo_script).encode()))],
#                                            executable_key="dynamo",
#                                            output_filenames=["script.dyn", OUTPUT_FILE_NAME])
#         generic_analysis.execute(timeout=30)
#     except TimeoutError:
#         raise UserException("Dynamo script got a timeout.")
#     except ConnectionError:
...
#     return generic_analysis.get_output_file(OUTPUT_FILE_NAME).getvalue().decode()