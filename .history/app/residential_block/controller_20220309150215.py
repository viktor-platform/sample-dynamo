	...
	def get_map_view(self, params: Munch, **kwargs) -> MapResult:
		...
		if params.tab1.section2.point:
			...
			for house in range(1,n_houses+1,1):
				...
				#Bottom left corner:
				if house==1: #Only computed for the case of 1 house
				...
				#Top right corner:
				Top_Right_x = center_x + ((width*(2*house) / 2) * cos(angle)) - ((height / 2) * sin(angle))
				Top_Right_y = center_y + ((width*(2*house) / 2) * sin(angle)) + ((height / 2) * cos(angle))
				coords.append((Top_Right_x,Top_Right_y))

				#Bottom right corner:
				Bot_Right_x = center_x + ((width*(2*house) / 2) * cos(angle)) + ((height / 2) * sin(angle))
				Bot_Right_y = center_y + ((width*(2*house) / 2) * sin(angle)) - ((height / 2) * cos(angle))
