...
def dynamojson_to_mesh(mesh_list: list):
    ...
    for mesh in mesh_list:
        print(mesh['GeometryEntries'].keys())
        if "TriangleVertices" not in mesh['GeometryEntries']:
            continue
        array = bytearray(base64.b64decode(mesh['GeometryEntries']["TriangleVertices"]))
        numbers = []
        vertices = []
        ...
        for _, _mesh in mesh_object.geometry.items():
