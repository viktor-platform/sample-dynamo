...
def dynamojson_to_mesh(mesh_list: list):
    objects = []
    for geometry_entries in mesh_list:
        for mesh in geometry_entries['GeometryEntries']:
            ...
            mesh_object = trimesh.Trimesh(vertices=vertices, faces=faces)
            objects.append(_Mesh(vertices=mesh_object.vertices.tolist(), faces=mesh_object.faces.tolist()))
            objects.append(_Mesh(vertices=mesh_object.vertices.tolist(),
                                faces=[list(reversed(e)) for e in mesh_object.faces.tolist()]))  # to visualize the other side
    return Group(objects)


def get_output_data(dynamo_output_names: List[str], dynamo_script: dict, dynamo_output_xml: bytes) -> dict:
    output_data = {}
    ...
    for dynamo_output_name in dynamo_output_names:
        output_id = output_ids[dynamo_output_name]
        output_data[dynamo_output_name] = output_results[output_id]
    return output_data
