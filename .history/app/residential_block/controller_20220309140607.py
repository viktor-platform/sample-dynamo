...
class ResidentialBlockController(ViktorController):
	...
	def get_map_view(self, params: Munch, **kwargs) -> MapResult:
		...
		if params.tab1.section2.point:
			...
			for house in range(1,2*n_houses+1,2):
				
				#TOP RIGHT VERTEX:
				Top_Right_x = center_x + ((width*house / 2) * cos(angle)) - ((height / 2) * sin(angle))
				Top_Right_y = center_y + ((width*house / 2) * sin(angle)) + ((height / 2) * cos(angle))
				...
				#BOTTOM RIGHT VERTEX:
				Bot_Right_x = center_x + ((width*house / 2) * cos(angle)) + ((height / 2) * sin(angle))
				Bot_Right_y = center_y + ((width*house / 2) * sin(angle)) - ((height / 2) * cos(angle))
				coords.append((Bot_Right_x,Bot_Right_y))

				map_point_list = []
				for coord in coords:
					coord_x_converted, coord_y_converted = RDWGSConverter.from_rd_to_wgs(coord)
					# features.append(MapPoint(coord_x_converted, coord_y_converted))
					map_point_list.append(MapPoint(coord_x_converted, coord_y_converted))

				polygon_coords = []
				for n, map_point in enumerate(map_point_list):
					polygon_coords = []
					if n>3:
						polygon_coords.append(map_point_list[n])
						polygon_coords.append(map_point_list[n-2])
						polygon_coords.append(map_point_list[n-2])
						polygon_coords.append(map_point_list[n])

						features.append(map_point_list[n])
						features.append(map_point_list[n-2])
						features.append(map_point_list[n-2])
						features.append(map_point_list[n])
					...
					# elif n<=3:
						# features.append(MapPolygon(map_point_list))
